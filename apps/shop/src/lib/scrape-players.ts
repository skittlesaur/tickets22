...
const scrapePlayers = async () => {
  try {
    ...
    for (const team of teams) {
      console.log(`Scraping ${team.name} players`)
      if (team.name.length < 3) break
      await page.goto(`https://www.fifa.com/fifaplus/en/tournaments/mens/worldcup/qatar2022/teams/${team.name === 'USA' ? 'united-states' : team.name.toLowerCase().replace(' ', '-')}/squad`)
