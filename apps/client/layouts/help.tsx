...
const components = {
  ...
  h2: ({ children }: any) => (
    <h2 className="font-bold text-2xl mt-6">{children}</h2>
  ),
  h3: ({ children }: any) => (
...
const HelpLayout = ({ children, title }: HelpProps) => {
  return (
    <div className="relative flex flex-col min-h-screen gap-16 max-w-full overflow-clip bg-gray-50 dark:bg-gray-900">
      <Seo title={title} />
      <div className="overflow-hidden bg-primary h-[40vh] fixed top-0 left-0 right-0 z-[0]">
        <div className="flex items-center justify-center">
          ...
          >
            ...
            <p className="font-mono font-medium text-xl opacity-80 ml-4">
              /help
            </p>
          </Link>
        </div>
      ...
      <div className="relative z-[1] mt-36 mb-24 bg-white max-w-screen-xl mx-auto w-full grow px-4 md:px-20 py-16 shadow-xl rounded-3xl flex flex-col md:flex-row gap-9 dark:bg-gray-980">
